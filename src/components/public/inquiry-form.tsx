...
export function InquiryForm({ projects, configured }: { projects: Project[]; configured: boolean }) {
  ...
  const { register, handleSubmit, reset, setError, clearErrors, formState: { errors } } = useForm<InquiryFields>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5" noValidate aria-busy={pending}>
      ...
      <label className="flex items-start gap-3 text-xs leading-6 text-body-copy-muted"><input {...register("consent", { required: "Consent is required." })} type="checkbox" className="mt-1 h-4 w-4 accent-deep-olive" />I agree that Stalwart Realtors may use these details to respond to my inquiry.</label>
      ...
      {!configured ? <p className="border-l-2 border-champagne-gold pl-4 text-xs leading-6 text-body-copy-muted">Online submissions will activate when the secure database is connected.</p> : null}
