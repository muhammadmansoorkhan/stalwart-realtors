...
export function SiteVisitForm({ projects, configured, initialProjectId = "" }: { projects: Project[]; configured: boolean; initialProjectId?: string }) {
  ...
  const { register, handleSubmit, reset, setError, formState: { errors } } = useForm<VisitFields>({ defaultValues: { projectId: initialProjectId, visitorCount: "", website: "" } });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5" noValidate>
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        <Field label="Preferred date" error={errors.preferredDate?.message}><input {...register("preferredDate", { required: "Choose a date." })} type="date" min={today} className="form-control" /></Field>
      ...
      <p className="text-xs leading-6 text-charcoal/60">Submitting this form does not confirm a visit. A representative will contact you to confirm availability.</p>
      ...
      {!available ? <p className="border-l-2 border-champagne-gold pl-4 text-xs leading-6 text-charcoal/60">Site-visit requests will activate when a verified project is published.</p> : null}
