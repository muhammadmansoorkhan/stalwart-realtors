    ...
    <>
      ...
      <section className="bg-soft-cream py-20 sm:py-28">
        <Container className="grid gap-12 lg:grid-cols-[1.25fr_0.75fr] lg:gap-20">
          <article><p className="text-xs font-bold tracking-[0.2em] text-deep-olive uppercase">Project overview</p><h2 className="mt-5 font-heading text-4xl leading-tight text-charcoal sm:text-5xl">{project.short_description}</h2><div className="prose-stalwart mt-8 text-base leading-8 text-charcoal/68">{project.overview.split(/\n\s*\n/).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></article>
...
function ListBlock({ title, items }: { title: string; items: string[] }) { return <div><h2 className="font-heading text-4xl text-charcoal">{title}</h2><ul className="mt-7 grid gap-3 sm:grid-cols-2">{items.map((item) => <li key={item} className="border-l border-champagne-gold py-1 pl-4 text-sm text-charcoal/70">{item}</li>)}</ul></div>; }
