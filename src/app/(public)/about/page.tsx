    ...
    <>
      ...
      <section className="section-reveal bg-soft-cream py-20 sm:py-28">
        <Container className="grid gap-12 lg:grid-cols-[0.75fr_1.25fr] lg:gap-20">
          ...
          <div className="prose-stalwart border-t border-deep-olive/15 pt-7 text-base leading-8 text-body-copy sm:text-lg sm:leading-9"><p>{settings.company_introduction}</p><p>The company’s positioning is centred on trust, transparency, strategic decisions, client-focused service, and the pursuit of long-term value. Specific claims and capabilities are published only after confirmation.</p></div>
        </Container>
      </section>
      {settings.mission || settings.vision ? (
        <section className="section-reveal border-y border-deep-olive/10 bg-warm-ivory py-20 sm:py-28">
          <Container className="grid gap-6 lg:grid-cols-2">
            ...
            {settings.vision ? <article className="border border-deep-olive/15 bg-soft-cream p-8 sm:p-12"><p className="text-xs font-bold tracking-[0.2em] text-deep-olive uppercase">Vision</p><h2 className="mt-5 font-heading text-4xl text-charcoal">Our stated direction.</h2><p className="mt-6 text-base leading-8 text-body-copy">{settings.vision}</p></article> : null}
          </Container>
        </section>
      ) : null}
      ...
      <section className="section-reveal bg-soft-cream py-20 sm:py-28">
        <Container><SectionHeading eyebrow="Three divisions" title="Distinct disciplines. Shared principles." /><div className="mt-12 grid gap-6 lg:grid-cols-3">{divisions.map((division) => { const Icon = division.icon; return <article key={division.slug} className="border border-deep-olive/12 p-8"><Icon className="text-champagne-gold" aria-hidden="true" size={30} strokeWidth={1.4} /><h3 className="mt-7 font-heading text-3xl text-charcoal">{division.name}</h3><p className="mt-4 text-sm leading-7 text-body-copy-muted">{division.summary}</p><ButtonLink href={`/${division.slug}`} variant="text" className="mt-6">Explore division</ButtonLink></article>; })}</div></Container>
      </section>
